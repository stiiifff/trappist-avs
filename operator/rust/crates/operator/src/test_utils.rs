...
async fn monitor_new_tasks() -> Result<()> {
    ...
    loop {
        ...
        let filter = Filter::new()
            .address(trappist_contract_address)
...
async fn register_operator() -> Result<()> {
    ...
    let stake_registry_address: Address =
        parse_stake_registry_address("contracts/deployments/trappist/31337.json")?;
    ...
    > = contract_ecdsa_stake_registry
        ...
        .gas(500000);
    let register_trappist_hash = registeroperator_details_call
        .send()
        .await?
        .get_receipt()
        .await?
...
async fn create_new_task(task_name: &str) -> Result<()> {
    ...
    let s = &format!(
        "{}/contracts/deployments/trappist/31337.json",
        &path.display()
    );
    let parsed: trappistData =
...
mod tests {
    ...
    async fn test_spam_tasks() {
        ...
        let parsed: trappistData = serde_json::from_str(&data).unwrap();
        let trappist_contract_address: Address = parsed
            .addresses
            .trappist_service_manager
            .parse()
            .unwrap();
        ...
        let latest_task_num_after_creating_task =
            trappist_contract.latestTaskNum().call().await.unwrap();
